...
function assignTorsoAttr() {
	var radios1 = document.getElementById("torso1").getElementsByTagName('input');
	for (var i=0; i<radios1.length; i++) {
		if (radios1[i].type == 'radio') {
			if (radios1[i].checked) {
				tattr1 = radios1[i].value;
			}
		}
	}
	var radios2 = document.getElementById("torso2").getElementsByTagName('input');
	for (var i=0; i<radios2.length; i++) {
		if (radios2[i].type == 'radio') {
			if (radios2[i].checked) {
				tattr2 = radios2[i].value;
			}
		}
	}
	var radios3 = document.getElementById("torso3").getElementsByTagName('input');
	for (var i=0; i<radios3.length; i++) {
		if (radios3[i].type == 'radio') {
			if (radios3[i].checked) {
				tattr3 = radios3[i].value;
			}
		}
	}

	var radios4 = document.getElementById("torso4").getElementsByTagName('input');
	for (var i=0; i<radios3.length; i++) {
		if (radios3[i].type == 'radio') {
			if (radios3[i].checked) {
				tattr4 = radios4[i].value;
			}
		}
	}
}

function assignLegsAttr() {
	var radios1 = document.getElementById("legs1").getElementsByTagName('input');
	for (var i=0; i<radios1.length; i++) {
		if (radios1[i].type == 'radio') {
			if (radios1[i].checked) {
				lattr1 = radios1[i].value;
			}
		}
	}
	var radios2 = document.getElementById("legs2").getElementsByTagName('input');
	for (var i=0; i<radios2.length; i++) {
		if (radios2[i].type == 'radio') {
			if (radios2[i].checked) {
				lattr2 = radios2[i].value;
			}
		}
	}
}
...
var legsCompiled = _.template(
	"<div class='legPost'>" +
	"<p><a><%= lattr1 %></a><a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp</a><a><%= lattr2 %></a></p>" +
	"</div>"
);
...
function refreshList() {
	characters.forEach( function(p, i) {
		...
		strTorso = torsoCompiled(p);
		strLegs = legsCompiled(p);
	})

	document.getElementById("list").innerHTML += str;
	document.getElementById("list").innerHTML += strHead;
	document.getElementById("list").innerHTML += strTorso;
	document.getElementById("list").innerHTML += strLegs;
}
